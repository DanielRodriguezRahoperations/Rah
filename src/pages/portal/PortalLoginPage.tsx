...
const PortalLoginPage = () => {
  ...
  useEffect(() => {
    ...
    if (tok) navigate('/portal/dashboard', { replace: true });
  }, [navigate]);
  ...
  const verifyCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        localStorage.setItem(PORTAL_TOKEN_KEY, json.token);
        localStorage.setItem(PORTAL_CLIENT_KEY, json.clientId);
  ...
  return (
    <>
      <SEOHead
        title="Client Portal Login — RAH Operations"
        description="Sign in to your RAH Operations client portal."
        noIndex={true}
      />
          ...
          <div className="bg-[#1a1a1a] border border-neutral-800 rounded-sm p-8 lg:p-10">
            <div className="text-center mb-8">
              ...
              <p className="text-sm text-neutral-400 font-serif-body">
                {stage === 'email'
                  ? 'Enter the email used to start your case.'
