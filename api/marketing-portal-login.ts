...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!supabaseUrl || !serviceKey || !resendKey) {
    console.error('[marketing-portal-login] missing env vars');
  ...
  if (cErr) {
    console.error('[marketing-portal-login] lookup error:', cErr);
  ...
  await supabase.from('marketing_portal_sessions').delete().eq('email', email);

  const { error: insErr } = await supabase.from('marketing_portal_sessions').insert({
    client_id: client.id,
    email,
    code,
  ...
  if (insErr) {
    console.error('[marketing-portal-login] insert error:', insErr);
...
<body style="margin:0;padding:0;background:#f4f4f4;font-family:Georgia,serif;">
    ...
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:2px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        <tr><td style="background:#0f0f0f;padding:28px 40px;text-align:center;">
          ...
          <p style="margin:8px 0 0;color:#888;font-size:10px;text-transform:uppercase;letter-spacing:4px;">Marketing Portal Login</p>
        </td></tr>
        <tr><td style="padding:36px 40px;">
          <p style="margin:0 0 16px;font-size:15px;color:#1a1a1a;">Hi ${firstName},</p>
          <p style="margin:0 0 24px;font-size:14px;color:#555;line-height:1.7;">Use the verification code below to sign in to your marketing portal. This code expires in 15 minutes.</p>
          ...
          <p style="margin:0;font-size:12px;color:#999;line-height:1.7;">If you did not request this code, you can safely ignore this email.</p>
        </td></tr>
  ...
  const emailRes = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [email],
      subject: `Your RAH Marketing Portal Code: ${code}`,
      html,
    }),
  });
  ...
  if (!emailRes.ok) {
    ...
    console.error('[marketing-portal-login] resend error:', err);
